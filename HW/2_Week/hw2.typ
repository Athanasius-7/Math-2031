#set page(
  ...
  header: context {
    if counter(page).get().first() > 1 [
      ...
      #grid(
        columns: (1fr, 1fr),
        align(left)[Imran Qasimi], align(right)[Math 2031 --- Homework 2],
      )
...
#question("21")[
  #given(
    "Statement",
    [Let $A$ and $B$ be sets contained in universal set $U$. Which one(s) of the following statements are equivalent to the statement $A subset.eq B?$ Give reasons for your answers. ],
  )
  #v(0.6em)
  #parts(
...
#question("1")[
  #given(
    "Given",
    [Let $A = {1,2,3,4,5}, B = {0,1,4,8}$ and $C = {2, 5, 7, 9, 11, 13, 17}$. Compute each of the following:],
  )
  #parts(
...
#question("2")[
  #given(
    "Given",
    [Let $A, B$ and $C$ be the sets given in the previous exercise and let the universal set $U={0,1,2,3,4,...,20}$. Compute the following:],
  )
...
#question("14B")[
  #given(
    "Statement",
    [Let $A$ and $B$ be sets. Prove the following: $A union (A inter B) = A$],
  )
  = Proof:
...
#question("17")[
  ...
  #parts(
    [  #given(
        "Conjecture",
        [$A subset.eq B union C arrow.r A subset.eq B or A subset.eq C$],
      )
      = Proof:
    ...
    [#given(
        "Conjecture",
        [$A subset.eq B or A subset.eq C arrow.r A subset.eq B union C$],
      )
      = Proof:
